...
make test
make up             # namenode, datanode, kafka, spark master+worker, hive, superset
```

Kaggle credentials (`KAGGLE_USERNAME`, `KAGGLE_KEY`) are only needed once ingest downloaders exist.
...
| Target | What |
|---|---|
| `make setup` | `uv lock`, `uv sync --all-groups`, create `data/*` dirs, print config |
| `make ingest` | `DATASET=all` (or `kaggle` / `bmrc` / `bmtc` / `mmrda` / `dgca`) |
| `make amplify` | `ROWS=1000000` (also 10M / 100M later) |
| `make up` / `make down` | Compose stack |
| `make hotspots` | Spark demand job **inside** the `spark` service |
| `make stream-produce` | `TOPIC=rides.raw RATE=500` (host → Kafka `:9094`) |
| `make stream-consume` | rolling metrics **inside** Compose Spark |
| `make hive-ddl` | beeline against `rides.sql` (needs stack up + Hive↔HDFS spike done) |
| `make mr-package` | Maven package in Docker (no host Java) |
...
- Host Python is 3.11. Spark jobs use the Bitnami Spark image, not host `spark-submit`.
